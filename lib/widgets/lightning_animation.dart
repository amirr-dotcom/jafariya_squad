...
class LightningPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final glowPaint = Paint()
      ..color = const Color(0xFFD32F2F).withOpacity(0.3 * glowProgress)
      ..strokeWidth = 8.0
      ..strokeCap = StrokeCap.round
      ..style = PaintingStyle.stroke;

    // Draw multiple lightning bolts
    _drawLightning(canvas, size, paint, glowPaint, 0.2);
    _drawLightning(canvas, size, paint, glowPaint, 0.8);
    _drawLightning(canvas, size, paint, glowPaint, 0.5);
  }

  void _drawLightning(Canvas canvas, Size size, Paint paint, Paint glowPaint, double offset) {
    final path = Path();
    final glowPath = Path();
    
    final startX = size.width * offset;
    final startY = 0.0;
    final endX = size.width * (offset + 0.1);
    final endY = size.height;
    
    final points = _generateLightningPoints(
  ...
  List<Offset> _generateLightningPoints(Offset start, Offset end, int segments) {
    ...
    for (int i = 1; i < segments; i++) {
      ...
      final offsetX = (random.nextDouble() - 0.5) * 50;
